 ...
 */
+(void)setConsumerKey:(NSString *)aKey consumerSecret:(NSString *)aSecret callbackURL:(NSString *)callbackURL __attribute__((deprecated("請改用 '+setConsumerKey:consumerSecret:callbackURL:'")));
/**
 ...
 *  @param passwd         如果指定使用者的 Blog 被密碼保護，則需要指定這個參數以通過授權
 *  @param page           頁數
 *  @param userCategories 要取回的類別 id 
 *  @param completion     succeed = YES 時 result 可以用，succeed = NO 時 result 會是 nil，錯誤原因會在 NSError 物件中
 */
...
- (void)updateBlogCommentOpenWithCommentID:(NSString *)commentID
                                    isOpen:(BOOL)isOpen
                                completion:(PIXHandlerCompletion)completion __deprecated_msg("use updateBlogCommentsOpenWithComments:isOpen:completion: instead of this.");
/**
*  將多則留言設為公開/關閉（需認證）
